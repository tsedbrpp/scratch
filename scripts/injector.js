...
if (!code.includes("export const GhostNodesPass1Schema")) {
  code = code.replace(
...
if (!code.includes("function buildPass1Prompt")) {
  code = code + "\n" + promptBuilders;
} else {
  // Wait, if it *is* in there, where is it?! Let's check if buildPass2Prompt is in there and overwrite it
  code = code.replace(/function buildPass1Prompt[\s\S]*?return result;\n}/, "");
  // just kidding let's not risk bad regex
}
