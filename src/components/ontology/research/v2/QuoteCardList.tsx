...
interface QuoteCardListProps {
    ...
    onContextRequest?: (quote: string) => void;
    onFullDocRequest?: () => void;
    documentLabel?: string;
}

export function QuoteCardList({ quotes, onContextRequest, onFullDocRequest, documentLabel }: QuoteCardListProps) {
    ...
    return (
        <div className="space-y-4">
            ...
            {quotes.map((quote) => (
                <Card key={quote.id} className="border-l-4 border-l-blue-500 shadow-sm overflow-hidden">
                    ...
                    <CardFooter className="bg-slate-50 border-t py-3 px-4 flex flex-col items-start gap-3 sm:flex-row sm:items-center sm:justify-between">
                        <div className="flex flex-col gap-2">
                            <div className="text-xs font-medium text-slate-500">
                                Source: {documentLabel || quote.sourceRef.docId} {quote.sourceRef.section ? `• ${quote.sourceRef.section}` : ''} {quote.sourceRef.paragraph ? `• ${quote.sourceRef.paragraph}` : ''}
                            </div>
                        ...
                        <div className="flex items-center gap-2 w-full sm:w-auto mt-2 sm:mt-0">
                            <Button
                                ...
                                onClick={() => onContextRequest && onContextRequest(quote.text)}
                            >
