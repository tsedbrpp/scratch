...
**Affected components**: TST meta-synthesis (both tracks), GNDP Pass 2/3.
...
The TST meta-synthesis operates on compressed summaries of prior analytical outputs. Information loss during compression may affect the granularity and nuance of findings. The compression logic aggressively strips raw evidence quotes, retaining only strategy-level descriptions and summary findings.

### 2.3 Parallel Architecture (TST)
TST Tracks 1 and 2 execute concurrently on the same context rather than sequentially. The structured extraction in Track 2 is not directly grounded in Track 1's qualitative reading within the same invocation. Consistency between outputs is achieved through shared context and post-hoc researcher review, not architectural dependency.

### 2.4 Jurisdiction Scope
The TST extraction prompt includes configurable jurisdiction-scope rules. Different prompt configurations may be required for different jurisdiction groupings. Cross-jurisdictional claims must be validated across all configurations.
...
| **Epistemic Partition** | Every proposition evaluation partitions evidence into grounded/inferred/unknown | GNDP Pass 3, TST |
...
| **Cross-Stratum Triangulation** | Convergent findings across independently generated strata carry greater weight | TST validation strategy |
...
| TST meta-synthesis (dual track) | $0.15–0.40 | 60–120 seconds |
