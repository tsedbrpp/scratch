"""
Extended Translational Stratification Diagram – 3‑Stage Pipeline + Outcome
Generates a PowerPoint slide visualizing:
1️⃣ Referential Drift
2️⃣ Infrastructure Embedding
3️⃣ Stratified Legibility
4️⃣ Outcome (Sedimentation vs. Counter‑Translation)
...
JURIS = [
    ...
    },
    {
        ...
        'outcome': 'Weak deterritorialization'
    },
    {
        'name': 'United States',
        'color': '#6B7280',
...
SLIDE_H = Inches(8.0)  # a bit taller for extra column
MARGIN = Inches(0.35)
USABLE_W = SLIDE_W - 2 * MARGIN

# Column widths (proportional)
COL_JUR = Inches(1.5)
COL_DRIFT = Inches(2.8)
COL_EMBED = Inches(2.8)
COL_LEGIBLE = Inches(2.8)
COL_OUTCOME = Inches(1.8)
GAP = Inches(0.08)
...
X_OUTCOME = X_LEGIBLE + COL_LEGIBLE + GAP
...
add_textbox(slide, MARGIN, Inches(0.12), USABLE_W, Inches(0.4),
            'Translational Stratification – 3‑Stage Pipeline + Outcome',
            size=20, bold=True, color=hex_rgb('#111827'))
add_textbox(slide, MARGIN, Inches(0.52), USABLE_W, Inches(0.35),
            'Stable vocabularies travel, local referents reshape governance, and outcomes diverge (sedimentation vs. counter‑translation).',
            size=10.5, color=hex_rgb('#6B7280'), italic=True)

# Header row (four columns + outcome)
...
rich_cell(slide, X_OUTCOME, MARGIN + HEADER_H, COL_OUTCOME, HEADER_H,
          hex_rgb('#FEF2F2'), hex_rgb('#FECACA'),
          [{'t': 'OUTCOME', 's': 9, 'b': True, 'c': '#991B1B', 'a': PP_ALIGN.CENTER}], radius=4)
...
for idx, jur in enumerate(JURIS):
    ...
    # Outcome cell (use distinct background for counter‑translation)
    outcome_bg = '#FDE68A' if jur['outcome'] == 'Counter‑Translation' else '#F9FAFB'
    outcome_bd = '#FBBF24' if jur['outcome'] == 'Counter‑Translation' else '#D1D5DB'
    rich_cell(slide, X_OUTCOME, y, COL_OUTCOME, ROW_H,
              hex_rgb(outcome_bg), hex_rgb(outcome_bd),
              [{'t': jur['outcome'], 's': 9, 'c': '#991B1B', 'a': PP_ALIGN.CENTER}], radius=5)
    # Dashed arrows (drift → embed → legible)
    add_arrow(slide,
...
# Bottom callout (same as previous diagram)
...
run1 = para.add_run()
run1.text = 'Stable vocabularies travel, but local referents create inequity through layered compliance infrastructures; outcomes diverge as sedimentation or counter‑translation.'
...
add_textbox(slide, MARGIN, legend_y, USABLE_W, Inches(0.2),
            'Legend: ↦ Dashed arrows show the flow from drift → embedding → legibility. Colours correspond to jurisdictions; orange outcome cells indicate counter‑translation.',
            size=9, color=hex_rgb('#6B7280'))

# Save PPTX
out_path = r'c:\\Users\\mount\\.gemini\\antigravity\\scratch\\Translational_Stratification_Extended.pptx'
prs.save(out_path)
